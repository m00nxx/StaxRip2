...
StaxRip2 is a portable application, meaning that it has no installer.
You only need to extract StaxRip2's folder from the downloaded archive, using [7-zip](https://www.7-zip.org) or
any archiver compatible with the 7-zip `7z` format. 

> :warning: For the destination you should choose a location where you,
as the user who executes StaxRip2, have write permissions to avoid issues with some tools.

Once the folder is extracted, you are ready to go and start `StaxRip2.exe` from inside the folder you have extracted.

StaxRip2 includes portable versions of AviSynth+, VapourSynth, and Python.
...
> [!CAUTION]
> **Windows 7** users can use StaxRip2 only partly. The following tools are included and don't have
