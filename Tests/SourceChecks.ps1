...
Assert-Contains $assemblyInfo 'AssemblyVersion("0.1.7")' "Assembly version must follow the StaxRip2 release line."
Assert-Contains $assemblyInfo 'AssemblyFileVersion("0.1.7")' "File version must follow the StaxRip2 release line."
...
Assert-Contains $updateChecker "JavaScriptSerializer" "Update checks must parse GitHub release JSON structurally."
Assert-Contains $updateChecker "DeserializeObject(content)" "Update checks must deserialize release JSON instead of scraping raw response text."
Assert-Contains $updateChecker "TryGetStringValue" "Update checks must tolerate missing GitHub JSON string fields."
Assert-Contains $updateChecker "TryGetObjectArrayValue" "Update checks must tolerate missing GitHub JSON asset arrays."
Assert-Contains $updateChecker "Version.TryParse" "Update checks must tolerate malformed release or dismissed version values."
Assert-Contains $updateChecker 'assetPlatforms.Contains(assetMatch.Groups("platform").Value' "Update checks must search all supported asset platforms for the current process."
Assert-Contains $updateChecker "(?<version>\d+\.\d+\.\d+" "Update asset matching must support multi-digit version components."
Assert-NotContains $updateChecker "Regex.Matches(content, linkPattern)" "Update checks must not scrape GitHub JSON with a release-link regex."
...
Assert-Contains $mainForm "If filter IsNot Nothing Then" "ModifyFilters must tolerate missing default source filters after corrupt profile settings."
Assert-Contains $mainForm "If fallbackFilter IsNot Nothing Then" "ModifyFilters must tolerate missing final source filter fallbacks."
Assert-Contains $mainForm "No source filter is available for the current script engine." "ModifyFilters must fail clearly when no source filter can be selected."
Assert-Contains $mainForm "sourceFilter = p.Script.GetFilter(""Source"")" "ModifyFilters must refresh the source filter after preference matching."
...
Assert-Contains $toolUpdate "ProcessHelp.KillProcessAndChildren(pr.Id)" "Tool update extraction must kill timed-out extractor process trees."
...
Assert-Contains $toolUpdate "MoveCurrentFilesToBackup" "Tool updates must move existing files to a rollback backup before copying replacements."
Assert-Contains $toolUpdate "RestoreBackup" "Tool updates must restore existing files if replacement copying fails."
Assert-Contains $toolUpdate "Tool update failed while replacing files. Existing files were restored." "Tool update failures must explain rollback behavior."
Assert-Contains $toolUpdate "CleanupExtractDir" "Tool update extraction directories must be cleaned on failed extraction paths."
Assert-Contains $toolUpdate "TryUpdatePackageVersion" "Tool update version metadata updates must be separated from file replacement rollback."
Assert-Contains $toolUpdate "Tool update completed, but updating the stored version or testing commands failed." "Tool update post-copy validation failures must not roll back copied files."
...
Assert-Contains $processHelp "Shared Sub KillProcessAndChildren" "Process helpers must expose shared process-tree termination."
Assert-Contains $processHelp "KillProcessAndChildren(proc.Id)" "Console output timeout must kill child processes, not only the direct process."
Assert-Contains $processHelp "Throw New TimeoutException" "Console output timeout failures must be explicit."
Assert-Contains $processHelp "If Not Task.WaitAll(streamTasks, timeoutMilliseconds) Then" "Console output helpers must timeout stream draining after process exit."
Assert-Contains $processHelp "Catch ex As Exception When TypeOf ex Is ArgumentException" "Process-tree termination must tolerate race and permission failures."
...
Assert-Contains $readme 'Do not open `.srip`, `.dat`, or settings files from untrusted sources.' "README must document the remaining trusted-file boundary for serialized project data."
...
Assert-Contains $changelog "v0.1.7 (2026-05-31)" "Changelog must include the v0.1.7 release entry."
Assert-Contains $changelog "Validated cached runtime payload archives" "Changelog must mention runtime payload checksum validation."
Assert-Contains $changelog "Separated tool replacement rollback" "Changelog must mention tool update rollback hardening."
Assert-Contains $changelog "Hardened update checks against malformed GitHub release data" "Changelog must mention update check parsing hardening."
Assert-Contains $changelog "v0.1.6 (2026-05-31)" "Changelog must include the v0.1.6 release entry."
Assert-Contains $changelog "Added runtime payload bootstrapping" "Changelog must mention reproducible release packaging from a clean checkout."
Assert-Contains $changelog "Made tool replacement transactional" "Changelog must mention transactional tool update replacement."
Assert-Contains $changelog "Preserved corrupt Jobs.dat files" "Changelog must mention corrupt job file preservation."
...
Assert-Contains $buildDocs "StaxRip2-v0.1.7-x64.7z" "Build docs must document the v0.1.7 release archive name."
Assert-Contains $buildDocs "-RequireFullRuntime" "Build docs must document full runtime package validation."
Assert-Contains $buildDocs "-RuntimePayloadUrl" "Build docs must document runtime payload bootstrapping from a previous release archive."
Assert-Contains $buildDocs "-RuntimePayloadArchive" "Build docs must document runtime payload bootstrapping from a local archive."
Assert-Contains $buildDocs "-RuntimePayloadSha256" "Build docs must document runtime payload checksum validation."
...
Assert-Contains $releaseScript 'GetFullPath($ArtifactsDirectory)' "Release packaging must normalize relative artifact directories before Push-Location."
Assert-Contains $releaseScript 'RuntimePayloadArchive' "Release packaging must support bootstrapping runtime assets from a local archive."
Assert-Contains $releaseScript 'RuntimePayloadUrl' "Release packaging must support bootstrapping runtime assets from a previous release URL."
Assert-Contains $releaseScript "RuntimePayloadSha256" "Release packaging must support checksum validation for runtime payload archives."
Assert-Contains $releaseScript "Test-RuntimePayloadArchiveChecksum" "Release packaging must validate cached runtime payload archives before reuse."
Assert-Contains $releaseScript ".download" "Release packaging must download runtime payload archives to a temporary file before cache reuse."
Assert-Contains $releaseScript "Import-RuntimePayload" "Release packaging must import runtime folders before runtime validation."
Assert-Contains $releaseScript "Download-RuntimePayloadArchive" "Release packaging must download runtime payload archives when requested."
Assert-Contains $releaseScript "Copy-RuntimePayloadDirectory" "Release packaging must copy runtime folders without replacing the freshly built executable."
Assert-Contains $releaseScript "/t:Rebuild" "Release packaging must rebuild so version changes are reflected in the package."
Assert-Contains $releaseScript "function Invoke-MSBuild" "Release packaging must invoke MSBuild through explicit process handling."
Assert-Contains $releaseScript "Start-Process -FilePath `$msbuild" "Release packaging must not rely on direct native command invocation for MSBuild."
...
Assert-Contains $releaseScript "Fonts\Default\Anuphan\Anuphan-VariableFont_wght.ttf" "Release packaging full runtime validation must include the configured default UI font."
Assert-Contains $releaseScript "Fonts\Code\Roboto_Mono\RobotoMono-VariableFont_wght.ttf" "Release packaging full runtime validation must include the configured code font."
...
Assert-Contains $smokePackage "Fonts\Default\Anuphan\Anuphan-VariableFont_wght.ttf" "Package smoke checks must validate the configured default UI font."
Assert-Contains $smokePackage "Fonts\Code\Roboto_Mono\RobotoMono-VariableFont_wght.ttf" "Package smoke checks must validate the configured code font."
...
Assert-Contains $releaseScript "[IO.FileShare]::ReadWrite" "Release archive readiness checks must tolerate antivirus or indexer readers."
...
Assert-Contains $smokePackage "function Invoke-PowerShellScript" "Package smoke test must invoke nested PowerShell scripts through explicit process handling."
Assert-Contains $smokePackage "Start-Process -FilePath `$powershellExe" "Package smoke test must not rely on direct native command invocation."
...
Assert-Contains $smokePackage "RuntimePayloadUrl" "Package smoke test must pass runtime payload URLs through to release packaging."
Assert-Contains $smokePackage "RuntimePayloadArchive" "Package smoke test must pass local runtime payload archives through to release packaging."
Assert-Contains $smokePackage "RuntimePayloadSha256" "Package smoke test must pass runtime payload checksums through to release packaging."
...
Assert-Contains $releaseNotes "# StaxRip2 v0.1.7" "Release notes must identify the v0.1.7 release."
Assert-Contains $releaseNotes "StaxRip2-v0.1.7-x64.7z" "Release notes must document the full package asset."
...
Assert-Contains $jobManager "PreserveCorruptJobsFile" "Corrupt job files must be preserved instead of disappearing silently."
Assert-Contains $jobManager "Jobs.dat.corrupt." "Corrupt job backups must be easy to identify."
Assert-NotContains $jobManager 'g.ShowException(ex, "Failed to load job file", jobsPath)' "Job load failures must not show duplicate dialogs before preserving corrupt jobs."
...
$eac3toForm = Read-RepoFile "Source/Forms/eac3toForm.vb"
Assert-Contains $eac3toForm "AnalyzeTimeoutMilliseconds" "eac3to analysis must use an explicit analysis timeout."
Assert-Contains $eac3toForm "RedirectStandardError = True" "eac3to analysis must drain stderr to avoid process deadlocks."
Assert-Contains $eac3toForm "BeginErrorReadLine()" "eac3to analysis must start asynchronous stderr draining."
